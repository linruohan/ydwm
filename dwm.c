...
void drawbar(Monitor *m) {
    ...
    if (m->isoverview) {
        ...
        for (i = 0; i < LENGTH(tags); i++) {
            ...
            w = TEXTW(tags[i]);
            drw_setscheme(
                drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSelTag
                                                           : SchemeNormTag]);
...
int drawstatusbar(Monitor *m, int bh, char *stext) {
    ...
    x = m->ww - w - system_w - 2 * sp -
        (system_w ? systrayspadding
                  : 0); // 托盘存在时 额外多-一个systrayspadding
...
void pointerfocuswin(Client *c) {
    if (c) {
        XWarpPointer(dpy, None, root, 0, 0, 0, 0, c->x + c->w / 2,
                     c->y + c->h / 2);
        focus(c);
    } else
    ...
    do {
        ...
        case MotionNotify:
            ...
            if (!c->isfloating &&
                ...
                c->isfloating = 1;
                arrange(selmon);
                if (ev.xmotion.x - nx < c->w / 2 &&
                    ev.xmotion.y - ny < c->h / 2 &&
...
void showtag(Client *c) {
    ...
    if (ISVISIBLE(c)) {
        ...
        if (c->mon->mx == 0) {
            ...
            XMoveWindow(dpy, c->win, c->mon->mx + c->mon->mw + WIDTH(c) * 1.5,
                        c->y);
        }
    }
}
        ...
        for (c = selmon->clients; c; c = c->next)
            if (ISVISIBLE(c) && !HIDDEN(c)) {
                ...
                resize(c, c->x + 2 * snap, c->y + 2 * snap,
                       MAX(c->w - 4 * snap, snap), MAX(c->h - 4 * snap, snap),
                       0);
            }
    }
    pointerfocuswin(selmon->sel);
...
void updatesystray(void) {
    ...
    if (!systray) {
        ...
        if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
            die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
        systray->win =
            XCreateSimpleWindow(dpy, root, x - sp, m->by + vp, w, bh, 0, 0,
...
void toggleoverview(const Arg *arg) {
    ...
    uint target = selmon->sel && selmon->sel->tags != TAGMASK
                      ? selmon->sel->tags
                      : selmon->seltags;
    selmon->isoverview ^= 1;
    view(&(Arg){.ui = target});
    ...
    while (1) {
        ...
        for (c = selmon->clients; c; c = c->next) {
            if (c->isglobal && c->tags == TAGMASK)
                continue;
            if (c->tags & target &&
                __builtin_popcount(selmon->tagset[selmon->seltags] & TAGMASK) ==
                    1 &&
                selmon->tagset[selmon->seltags] > 1) {
                view(&(Arg){.ui = target});
                return;
...
void grid(Monitor *m, uint gappo, uint gappi) {
    ...
    if (n == 1) {
        ...
        resize(c, m->mx + (m->mw - cw) / 2 + gappo,
               m->my + (m->mh - ch) / 2 + gappo, cw - 2 * c->bw, ch - 2 * c->bw,
               0);
        return;
    }
    if (n == 2) {
...
Client *direction_select(const Arg *arg) {
    ...
    if (tc && tc->isfullscreen) /* no support for focusstack with fullscreen
                                   windows */
        return NULL;
    if (!tc)
        tc = selmon->clients;
