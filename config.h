...
static Key keys[] = {
    ...
    {MODKEY, XK_p, spawn, SHCMD("$DWM/rofi.sh")}, /* super p          |
                                                         rofi: 执行自定义脚本 */
    {MODKEY, XK_n, spawn,
     SHCMD("$DWM/blurlock.sh")}, /* super n          | 锁定屏幕 */
    {MODKEY | ShiftMask, XK_Up, spawn,
     SHCMD("$DWM/set_vol.sh up")}, /* super shift up   | 音量加 */
    {MODKEY | ShiftMask, XK_Down, spawn,
     SHCMD("$DWM/set_vol.sh down")}, /* super shift down | 音量减 */
    ...
    TAGKEYS(XK_1, 0, 0) TAGKEYS(XK_2, 1, 0) TAGKEYS(XK_3, 2, 0)
        TAGKEYS(XK_9, 3, "obs") TAGKEYS(XK_c, 4, "google-chrome-stable")
            TAGKEYS(XK_m, 5, "$DWM/scripts/music_player.sh") TAGKEYS(XK_0, 6,
                                                                     "linuxqq")
                TAGKEYS(XK_w, 7, "/opt/apps/com.qq.weixin.deepin/files/run.sh")
                    TAGKEYS(XK_y, 8,
                            "/opt/apps/com.qq.weixin.work.deepin/files/"
                            "run.sh")};
...
static Button buttons[] = {
    ...
    {ClkStatusText,
     ...
     {0}}, // 左键        |  点击状态栏   |  根据状态栏的信号执行
           // $DWM/scripts/dwmstatusbar.sh $signal L
    {ClkStatusText,
     0,
     Button2,
     clickstatusbar,
     {0}}, // 中键        |  点击状态栏   |  根据状态栏的信号执行
           // $DWM/scripts/dwmstatusbar.sh $signal M
    {ClkStatusText,
     0,
     Button3,
     clickstatusbar,
     {0}}, // 右键        |  点击状态栏   |  根据状态栏的信号执行
           // $DWM/scripts/dwmstatusbar.sh $signal R
    {ClkStatusText,
     0,
     Button4,
     clickstatusbar,
     {0}}, // 鼠标滚轮上  |  状态栏       |  根据状态栏的信号执行
           // $DWM/scripts/dwmstatusbar.sh $signal U
    {ClkStatusText,
     0,
     Button5,
     clickstatusbar,
     {0}}, // 鼠标滚轮下  |  状态栏       |  根据状态栏的信号执行
           // $DWM/scripts/dwmstatusbar.sh $signal D
           //
    /* 点击bar空白处 */
    {ClkBarEmpty, 0, Button1, spawn,
     SHCMD("$DWM/scripts/call_rofi.sh window")}, // 左键        |  bar空白处 |
                                                 // rofi 执行 window
    {ClkBarEmpty, 0, Button3, spawn,
     SHCMD("$DWM/scripts/call_rofi.sh drun")}, // 右键        |  bar空白处    |
                                               // rofi 执行 drun
};
